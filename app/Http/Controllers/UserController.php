...
//            $destinationPath = './public/uploads/users/';
            ...
            return redirect('user/register')->with(['message'=>'ไม่มีรูปบัตรประชาชน/ข้าราชการ','status'=>'success'])
		...
		if ($validator->passes()) {
			...
			$authen = new User;
			$authen->username = trim($request->input('email'));
			...
			$authen->department_id = $request->input('department_id');
			$authen->province_id = $request->input('province_id');
			...
			{ 
				...
				$message = "ขอบคุณที่ลงทะเบียน ทางเราจะตรวจข้อมูลเพื่ออนุมัติบัญชีผู้ใช้ให้ครับ";
			} else {
	...
	public function getProfile() {
        ...
        $location = location::where("LOC_CODE",$info->province_id)->first();
        if ($location){
            $location = $location->LOC_PROVINCE;
        }else{
            $location = "";
        }
