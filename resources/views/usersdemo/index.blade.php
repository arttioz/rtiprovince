...
{{--	<h2> {{ $pageTitle }} <small> {{ $pageNote }} </small></h2>--}}
	<h1 class="title-head-users">ข้อมูลผู้ใช้งานระบบ</h1>
{{--	<ol class="breadcrumb">--}}
	...
	<div class="page-content-wrapper no-margin">
		...
		<div class="sbox">
			...
			<div class="row">
				<div class="col-md-4">
					@if($access['is_add'] ==1)
					<a href="{{ url('usersdemo/create?return='.$return) }}" class="btn btn-default btn-sm"
						title="{{ __('core.btn_create') }}"><i class=" fa fa-plus "></i> เพิ่มผู้ใช้งาน </a>
					@endif

					<div class="btn-group">
						<button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="icon-menu5"></i> : เมนู </button>
				...
				<div class="col-md-4 pull-right">
					<div class="input-group">
					      <div class="input-group-btn">
					        ...
					        onclick="SximoModal('{{ url($pageModule."/search") }}','Advance Search'); " ><i class="fa fa-search"></i> ค้นหา </button>
					      </div><!-- /btn-group -->
					      <input type="text" class="form-control input-sm onsearch" data-target="{{ url($pageModule) }}" aria-label="..." placeholder=" ระบุคำที่จะค้นหาแล้วกด Enter ">
					    </div>
				</div>
			</div>
			<!-- End Toolbar Top -->
			...
			<div class="table-responsive" style="padding-bottom: 70px;">
 			{!! Form::open(array('url'=>'usersdemo?'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'SximoTable' )) !!}

		    <table class="table table-striped table-hover " id="{{ $pageModule }}Table">
		        <thead>
					<tr>
						<th style="width: 3% !important;" class="number"> ลำดับที่ </th>
							...
							<td>
							 	...
							 	<div class="dropdown">
								  ...
								  <ul class="dropdown-menu">
									...
									<li><a href="{{ url('usersdemo/'.$row->id.'?return='.$return)}}" class="tips" title="{{ __('core.btn_view') }}"> {{ __('core.btn_view') }} </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li><a  href="{{ url('usersdemo/'.$row->id.'/edit?return='.$return) }}" class="tips" title="{{ __('core.btn_edit') }}"> {{ __('core.btn_edit') }} </a></li>
									@endif
